...
import Value, {ValueSegment} from "../../../src/compiler/parser/Value.js";
...
describe("String value test", () => {
    ...
    test(
        "simple string",
        new ValueSegment(false, "simple string")
    )
    test(
        "simple {{with template}}",
        new ValueSegment(false, "simple "),
        new ValueSegment(true, "with template")
    )
    test(
    ...
    test(
        "{{ just template }}",
        new ValueSegment(true, "just template"),
    )
})